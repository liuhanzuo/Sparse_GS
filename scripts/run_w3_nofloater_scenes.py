"""W3-nofloater driver — 8 scenes, floater-prune disabled.

诊断观察：当 train PSNR 拟合到 35+ dB 时（hotdog/materials），
SparseGS-style floater-prune 的判据无法区分"真 floater"和"高频细节
几何"，会把合法表面 Gaussian 误杀，导致 val PSNR 暴跌（hotdog 25.0 -> 19.3）。

本配置组关闭 floater-prune，保留：
  * W2 unseen-prune（永远是无害有益的）
  * dual-GS co-prune + co-reg（CoR-GS 风格双场拟合）
  * 其余沿用 W3 dual-GS

预期：拟合较好的场景（hotdog/chair/ficus/lego/mic/materials）回升，
      drums / ship 等本就 floater 严重的场景小幅下降，整体 8 场景平均提升。
"""
...
def cfg_for(scene: str) -> Path:
    fname = f"lego_n8_w3_nofloater.yaml" if scene == "lego" else f"blender_{scene}_n8_w3_nofloater.yaml"
    return ROOT / "configs" / "_w3_nofloater" / fname


def expname_for(scene: str) -> str:
    return f"lego_n8_w3_nofloater" if scene == "lego" else f"blender_{scene}_n8_w3_nofloater"
...
def run(scene: str) -> int:
    cfg = cfg_for(scene)
    log = LOGS / f"w3nofloater_{scene}.log"
...
def main() -> int:
    ...
    print("\n==================================================")
    print("W3-nofloater 8-scene driver summary:")
