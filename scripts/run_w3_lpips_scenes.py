"""W3-lpips driver — 8 scenes, with differentiable LPIPS-VGG perceptual loss.

W3 dual-GS + perceptual term on top:
  * lpips PyPI package, VGG backbone (frozen, gradients flow only into render)
  * weight=0.05, every=4 steps, start_iter=1000, patch_size=256
  * see configs/_w3_lpips/*.yaml for full settings

动机：纯 L1+SSIM 在稀疏视图下对半透明 floater / 边缘鬼影几乎不敏感
（像素差异小但视觉刺眼）；LPIPS-VGG 特征对高频几何错误极其敏感，
是 SOTA 方法的常见 regularizer (FreeNeRF / FSGS / SparseGS)。
"""
...
def cfg_for(scene: str) -> Path:
    fname = f"lego_n8_w3_lpips.yaml" if scene == "lego" else f"blender_{scene}_n8_w3_lpips.yaml"
    return ROOT / "configs" / "_w3_lpips" / fname


def expname_for(scene: str) -> str:
    return f"lego_n8_w3_lpips" if scene == "lego" else f"blender_{scene}_n8_w3_lpips"
...
def main() -> int:
    ...
    print("\n==================================================")
    print("W3-lpips 8-scene driver summary:")
